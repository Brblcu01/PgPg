...
type TokenMap = Record<string, TokenEntry> & {
    ...
    __strictCache?: Map<string, unknown>;
};
...
declare const dt: (tokenPath?: string, fallback?: string, type?: DtType) => string | number | unknown;
declare const dtwt: (theme?: ThemeConfig, tokenPath?: string, fallback?: string, type?: DtType) => string | number | unknown;

declare const $t: (theme?: ThemeConfig) => {
    preset(value: Preset): /*elided*/ any;
    options(value: ThemeOptions): /*elided*/ any;
    primaryPalette(primary: TokenObject): /*elided*/ any;
...
declare function getVariableValue(value: TokenValue, variable?: string, prefix?: string, excludedKeyRegexes?: RegExp[], fallback?: string): string | number | undefined;
...
/**
 * A node in a user-authored token tree: either a {@link TokenValue} leaf or a nested object of
 * more tokens. Preset trees are arbitrarily deep, so a container's values are themselves
 * `TokenValue | TokenObject`.
 *
 * @example
...
interface Preset {
     ...
     */
    components?: Record<string, TokenObject>;
    /**
     * Per-directive token overrides, keyed by directive name (e.g. `tooltip`, `ripple`).
     */
    directives?: Record<string, TokenObject>;
    /**
