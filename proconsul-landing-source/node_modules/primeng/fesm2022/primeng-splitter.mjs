...
import { Injectable, inject, input, numberAttribute, booleanAttribute, viewChild, ChangeDetectionStrategy, ViewEncapsulation, Component, output, contentChildren, computed, signal, effect, untracked, NgModule } from '@angular/core';
...
class SplitterStyle extends BaseStyle {
    ...
    style = style;
    classes = classes;
    static ɵfac = i0.ɵɵngDeclareFactory({ minVersion: "12.0.0", version: "22.0.5", ngImport: i0, type: SplitterStyle, deps: null, target: i0.ɵɵFactoryTarget.Injectable });
    static ɵprov = i0.ɵɵngDeclareInjectable({ minVersion: "12.0.0", version: "22.0.5", ngImport: i0, type: SplitterStyle });
}
i0.ɵɵngDeclareClassMetadata({ minVersion: "12.0.0", version: "22.0.5", ngImport: i0, type: SplitterStyle, decorators: [{
            type: Injectable
        }] });
/**
 *
...
(function (SplitterClasses) {
     ...
     */
    SplitterClasses["panel"] = "p-splitter-panel";
    /**
...
class SplitterPanel extends BaseComponent {
    componentName = 'SplitterPanel';
    bindDirectiveInstance = inject(Bind, { self: true });
    /**
     * Initial size of the panel relative to 100%.
     * @group Props
     */
    size = input(undefined, { ...(ngDevMode ? { debugName: "size" } : /* istanbul ignore next */ {}), transform: (value) => (value == null ? undefined : numberAttribute(value)) });
    /**
     ...
     * @group Props
     */
    collapsedSize = input(0, { ...(ngDevMode ? { debugName: "collapsedSize" } : /* istanbul ignore next */ {}), transform: numberAttribute });
    /**
     * @ignore
...
i0.ɵɵngDeclareClassMetadata({ minVersion: "12.0.0", version: "22.0.5", ngImport: i0, type: SplitterPanel, decorators: [{
    ...
    `,
                    ...
                    hostDirectives: [Bind],
                    providers: [SplitterStyle, { provide: PARENT_INSTANCE, useExisting: SplitterPanel }]
                }]
        }], propDecorators: { size: [{ type: i0.Input, args: [{ isSignal: true, alias: "size", required: false }] }], minSize: [{ type: i0.Input, args: [{ isSignal: true, alias: "minSize", required: false }] }], maxSize: [{ type: i0.Input, args: [{ isSignal: true, alias: "maxSize", required: false }] }], collapsible: [{ type: i0.Input, args: [{ isSignal: true, alias: "collapsible", required: false }] }], collapsedSize: [{ type: i0.Input, args: [{ isSignal: true, alias: "collapsedSize", required: false }] }], contentTemplate: [{ type: i0.ViewChild, args: ['content', { isSignal: true }] }] } });
...
class Splitter extends BaseComponent {
     ...
     * @group Props
     */
    gutterSize = input(1, { ...(ngDevMode ? { debugName: "gutterSize" } : /* istanbul ignore next */ {}), transform: numberAttribute });
    /**
     ...
     * @ignore
     */
    panelComponents = contentChildren(SplitterPanel, /* @ts-ignore */
    ...(ngDevMode ? [{ debugName: "panelComponents" }] : /* istanbul ignore next */ []));
    /**
     ...
     */
    panels = computed(() => {
        const components = this.panelComponents();
        if (components && components.length) {
            const templates = components.map((panel) => panel.contentTemplate());
            return templates.every((template) => !!template) ? templates : [];
        }
    ...
    panelMetas = computed(() => {
        ...
        const count = this.panelChildren().length;
    ...
    ...(ngDevMode ? [{ debugName: "panelMetas" }] : /* istanbul ignore next */ []));
    /**
     ...
     */
    panelSignature = computed(() => {
        const metaSig = this.panelMetas()
            .map((meta) => `${meta.size ?? ''}:${meta.minSize}:${meta.maxSize}:${meta.collapsible}:${meta.collapsedSize}`)
            .join(',');
        return `${metaSig}#${this.panelSizes().join(',')}`;
    }, /* @ts-ignore */
    ...(ngDevMode ? [{ debugName: "panelSignature" }] : /* istanbul ignore next */ []));
    /**
    ...
    panelClass = computed(() => {
        const components = this.panelComponents();
        const base = this.cn(this.cx('panel'), this.panelStyleClass());
        return this.panels().map((_, i) => this.cn(base, components[i]?.hostClass));
    }, /* @ts-ignore */
     ...
     */
    panelInlineStyle = computed(() => {
        const components = this.panelComponents();
        const fallback = this.panelStyle();
        return this.panels().map((_, i) => components[i]?.hostStyle ?? fallback);
    }, /* @ts-ignore */
    ...(ngDevMode ? [{ debugName: "panelInlineStyle" }] : /* istanbul ignore next */ []));
    dragging = false;
    isResizing = signal(false, /* @ts-ignore */
    ...(ngDevMode ? [{ debugName: "isResizing" }] : /* istanbul ignore next */ []));
    resizingIndex = signal(null, /* @ts-ignore */
    ...(ngDevMode ? [{ debugName: "resizingIndex" }] : /* istanbul ignore next */ []));
    collapsed = signal([], /* @ts-ignore */
     ...
     */
    panelFlex = signal([], /* @ts-ignore */
    ...(ngDevMode ? [{ debugName: "panelFlex" }] : /* istanbul ignore next */ []));
    dataResizing = computed(() => (this.isResizing() ? '' : null), /* @ts-ignore */
    ...(ngDevMode ? [{ debugName: "dataResizing" }] : /* istanbul ignore next */ []));
    pointerMoveListener;
    pointerUpListener;
    size;
    gutterElement;
    ...
    prevSize;
    _componentStyle = inject(SplitterStyle);
    rootClass = computed(() => this.cn(this.cx('root'), this.styleClass()), /* @ts-ignore */
    ...(ngDevMode ? [{ debugName: "rootClass" }] : /* istanbul ignore next */ []));
    constructor() {
        super();
        effect(() => {
            // Re-initialize whenever the set of panels or their (value-based) size configuration changes.
            const panels = this.panels();
            this.panelSignature();
            if (!isPlatformBrowser(this.platformId) || panels.length === 0) {
                return;
            }
            // Never clobber sizes mid-drag; the resizing flag is read untracked so ending a drag
            // does not by itself re-trigger initialization.
            if (untracked(() => this.isResizing())) {
                return;
            }
    ...
    buildPanelRegistry() {
        const metas = this.panelMetas();
        const prev = this.panelRegistry ?? [];
        this.panelRegistry = metas.map((meta, i) => ({
            index: i,
            minSize: meta.minSize ?? 0,
            maxSize: meta.maxSize ?? 100,
            collapsible: meta.collapsible ?? false,
            collapsedSize: meta.collapsedSize ?? 0,
            size: meta.size,
            collapsed: prev[i]?.collapsed ?? false
        }));
    }
    initializePanels() {
        const panels = this.panels();
        ...
        if (!initialized) {
            ...
            panels.forEach((_, i) => {
                const meta = this.panelRegistry[i];
                // Splitter-level `panelSizes` takes precedence, then the panel's own `size`, then an equal split.
                _panelSizes[i] = inputPanelSizes[i] ?? meta?.size ?? 100 / panels.length;
            });
            this.panelRegistry.forEach((info, i) => {
                if (info?.collapsible) {
                    info.collapsed = _panelSizes[i] <= (info.collapsedSize ?? 0);
                }
            });
    ...
    updateFlex() {
        this.panelFlex.set(this._panelSizes.map((panelSize) => panelSize + ' 1 0px'));
    }
    syncCollapsedState() {
        this.collapsed.set(this.panelRegistry.map((info) => info.collapsed));
    }
    ...
    resizeStart(event, index, isKeyDown) {
        ...
        if (currentTarget) {
            this.gutterElement = currentTarget;
        }
        const panelElements = this.el.nativeElement.querySelectorAll(':scope > [data-index]');
        let totalPanelPx = 0;
        for (const el of panelElements) {
            const rect = el.getBoundingClientRect();
            totalPanelPx += this.horizontal() ? rect.width : rect.height;
        }
        this.size = totalPanelPx;
        if (!isKeyDown) {
            const pointerEvent = event;
            this.dragging = true;
            this.startPos = this.horizontal() ? pointerEvent.pageX : pointerEvent.pageY;
            this.isResizing.set(true);
            this.resizingIndex.set(index);
        }
        this.initialSizes = [...this._panelSizes];
    ...
    onResizeMove(event, step, isKeyDown) {
        ...
        let delta;
        if (isKeyDown) {
            delta = (total * (step ?? 0)) / this.size;
        }
        else {
            const pointerEvent = event;
            const pos = this.horizontal() ? pointerEvent.pageX : pointerEvent.pageY;
            if (this.horizontal() && isRTL(this.el.nativeElement)) {
                delta = (((this.startPos ?? 0) - pos) * total) / this.size;
            }
            else {
                delta = ((pos - (this.startPos ?? 0)) * total) / this.size;
            }
        }
        const newSizes = [...this.initialSizes];
        this.panelRegistry.forEach((info, i) => {
            if (this.initialCollapsed[i] !== undefined) {
                info.collapsed = this.initialCollapsed[i];
            }
        });
        if (delta > 0) {
        ...
        this.panelRegistry.forEach((info, i) => {
            if (info.collapsed !== this.reportedCollapsed[i]) {
                ...
                this.onCollapse.emit({ index: i, collapsed: info.collapsed, sizes: [...newSizes] });
            }
        });
        this._panelSizes = newSizes;
        this.prevSize = newSizes[gutterIdx].toFixed(4);
        this.updateFlex();
        this.syncCollapsedState();
        this.onResize.emit({ originalEvent: event, sizes: [...newSizes] });
    }
    distribute(newSizes, growIdx, requested, isKeyDown, direction) {
        ...
        if (growInfo?.collapsible && growInfo.collapsed) {
            // Re-open symmetrically: any outward drag/step pops a collapsed panel back to (at least) its minimum size.
    ...
    validatePanelSize(rawSize, info, min, isKeyDown) {
        ...
        // The panel stops shrinking at its minimum size; the moment the user keeps dragging (or
        // stepping) past that minimum, a collapsible panel snaps to its collapsed state.
    ...
    repeat(event, index, step) {
        ...
        this.onResizeMove(event, step, true);
    }
    ...
    bindPointerListeners() {
        if (!this.pointerMoveListener) {
            this.pointerMoveListener = this.renderer.listen(this.document, 'pointermove', (event) => {
                this.onResizeMove(event);
            });
        }
        if (!this.pointerUpListener) {
            this.pointerUpListener = this.renderer.listen(this.document, 'pointerup', (event) => {
                this.resizeEnd(event);
    ...
    restoreState() {
        ...
        if (stateString) {
            ...
            this.panelRegistry.forEach((info, i) => {
                if (info?.collapsible) {
                    info.collapsed = this._panelSizes[i] <= (info.collapsedSize ?? 0);
                }
            });
            this.prevSize = parseFloat(String(this._panelSizes[0])).toFixed(4);
            return true;
        }
        return false;
    }
    ...
    gutterInlineStyle() {
        return {
            'touch-action': 'none',
            'user-select': 'none',
            ...(this.horizontal() ? { width: this.gutterSize() + 'px' } : { height: this.gutterSize() + 'px' })
        };
    }
    horizontal() {
        ...
        @for (panel of panels(); track $index; let i = $index) {
            ...
            @if (i !== panels().length - 1) {
                <div
                    ...
                    [attr.aria-valuemin]="round(panelMetas()[i]?.minSize)"
                    [attr.aria-valuemax]="round(100 - (panelMetas()[i + 1]?.minSize ?? 0))"
                    [attr.aria-disabled]="disabled() || null"
                    [attr.data-orientation]="layout()"
                    [attr.data-resizing]="isResizing() && resizingIndex() === i ? '' : null"
                    [attr.data-disabled]="disabled() ? '' : null"
                    [style]="gutterInlineStyle()"
                    (pointerdown)="onGutterPointerDown($event, i)"
                    (keyup)="onGutterKeyUp($event)"
                    (keydown)="onGutterKeyDown($event, i)"
                >
                    <div [pBind]="ptm('gutterHandle')" [class]="cx('gutterHandle')" [attr.data-orientation]="layout()" [attr.data-resizing]="isResizing() && resizingIndex() === i ? '' : null"></div>
                </div>
            }
        }
...
i0.ɵɵngDeclareClassMetadata({ minVersion: "12.0.0", version: "22.0.5", ngImport: i0, type: Splitter, decorators: [{
            type: Component,
            args: [{
                    selector: 'p-splitter',
                    standalone: true,
    ...
    `,
                    ...
                    host: {
                        '[class]': 'rootClass()',
                        '[attr.data-orientation]': 'layout()',
                        '[attr.data-resizing]': 'dataResizing()',
                        '[attr.data-disabled]': "disabled() ? '' : null",
                        '[attr.data-p]': 'dataP()'
                    },
