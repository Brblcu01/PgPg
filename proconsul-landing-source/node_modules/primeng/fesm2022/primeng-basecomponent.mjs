...
import { Injectable, InjectionToken, inject, PLATFORM_ID, ElementRef, Injector, ChangeDetectorRef, Renderer2, input, isSignal, signal, computed, effect, Directive } from '@angular/core';
...
class BaseComponentStyle extends BaseStyle {
    name = 'common';
    static ɵfac = i0.ɵɵngDeclareFactory({ minVersion: "12.0.0", version: "22.0.5", ngImport: i0, type: BaseComponentStyle, deps: null, target: i0.ɵɵFactoryTarget.Injectable });
    static ɵprov = i0.ɵɵngDeclareInjectable({ minVersion: "12.0.0", version: "22.0.5", ngImport: i0, type: BaseComponentStyle, providedIn: 'root' });
}
i0.ɵɵngDeclareClassMetadata({ minVersion: "12.0.0", version: "22.0.5", ngImport: i0, type: BaseComponentStyle, decorators: [{
            type: Injectable,
...
class BaseComponent {
     ...
     */
    dt = input(/* @ts-ignore */
    ...(ngDevMode ? [undefined, { debugName: "dt" }] : /* istanbul ignore next */ []));
    /**
     * Indicates whether the component should be rendered without styles.
     * @defaultValue undefined
     * @group Props
     */
    unstyled = input(/* @ts-ignore */
    ...(ngDevMode ? [undefined, { debugName: "unstyled" }] : /* istanbul ignore next */ []));
    /**
     ...
     * @defaultValue undefined
     */
    ptOptions = input(/* @ts-ignore */
    ...(ngDevMode ? [undefined, { debugName: "ptOptions" }] : /* istanbul ignore next */ []));
    /******************** Computed ********************/
    ...
    directivePT = signal(undefined, /* @ts-ignore */
    ...(ngDevMode ? [{ debugName: "directivePT" }] : /* istanbul ignore next */ []));
    directiveUnstyled = signal(undefined, /* @ts-ignore */
    ...(ngDevMode ? [{ debugName: "directiveUnstyled" }] : /* istanbul ignore next */ []));
    $unstyled = computed(() => {
    ...
    _$styleCache;
    get $style() {
        // Memoized: $style is read on every cx()/sx() call (and per ptm()/style guard) and
        // otherwise spreads several objects each time. Its inputs (_componentStyle and the
        // host instance's $style) are stable per instance. Cache cleared in ngOnInit so a
        // value resolved before subclass fields (e.g. _componentStyle) are set is recomputed (#19589).
        if (!this._$styleCache) {
            this._$styleCache = { theme: undefined, css: undefined, classes: undefined, inlineStyles: undefined, ...(this._getHostInstance(this) || {}).$style, ...this['_componentStyle'] };
        }
    ...
    _$paramsCache;
    get $params() {
        // Memoized: $params is read many times per component per change-detection cycle
        // (every ptm()/cx()/$pt access) and each call otherwise allocates two objects plus
        // a parent-chain walk. instance is constant and the resolved parent instance is
        // stable once the component is initialized, so cache it. The cache is cleared at the
        // start of ngOnInit so any value resolved during construction (before subclass
        // identity fields like componentName/hostName are set) is recomputed correctly (#19589).
     ...
     */
    ngOnInit() {
        // Drop any $params/$style values computed during construction (before subclass
        // identity/style fields such as componentName/hostName/_componentStyle are
        // initialized) so they are recomputed correctly now that the component is fully constructed.
        this._$paramsCache = undefined;
        this._$styleCache = undefined;
        this._loadCoreStyles();
        this._loadStyles();
        this.onInit();
        this._hook('onInit');
    ...
    _loadScopedThemeStyles(preset) {
        if (this.config?.theme()?.options?.cssVariables === false && this.$style?.name) {
            const added = Theme.addScopedToken({ [this.$style.name]: preset });
            if (added) {
                Theme.deleteLoadedStyleName(this.$style.name);
                this._loadThemeStyles();
            }
        }
    ...
    static ɵfac = i0.ɵɵngDeclareFactory({ minVersion: "12.0.0", version: "22.0.5", ngImport: i0, type: BaseComponent, deps: [], target: i0.ɵɵFactoryTarget.Directive });
    static ɵdir = i0.ɵɵngDeclareDirective({ minVersion: "17.1.0", version: "22.0.5", type: BaseComponent, isStandalone: true, inputs: { dt: { classPropertyName: "dt", publicName: "dt", isSignal: true, isRequired: false, transformFunction: null }, unstyled: { classPropertyName: "unstyled", publicName: "unstyled", isSignal: true, isRequired: false, transformFunction: null }, pt: { classPropertyName: "pt", publicName: "pt", isSignal: true, isRequired: false, transformFunction: null }, ptOptions: { classPropertyName: "ptOptions", publicName: "ptOptions", isSignal: true, isRequired: false, transformFunction: null } }, providers: [BaseComponentStyle, BaseStyle], usesOnChanges: true, ngImport: i0 });
}
i0.ɵɵngDeclareClassMetadata({ minVersion: "12.0.0", version: "22.0.5", ngImport: i0, type: BaseComponent, decorators: [{
            type: Directive,
            args: [{
                    standalone: true,
